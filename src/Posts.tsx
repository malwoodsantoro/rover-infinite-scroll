...
const Posts = () => {
  ...
  const fetchPosts = async (page: number, rover: string): Promise<Posts> => {
    const data = await fetch(
      `https://api.nasa.gov/mars-photos/api/v1/rovers/${rover}/photos?sol=1000&page=${page}&api_key=` +
  ...
  const loadMorePosts = () => {
    ...
    setLoading(true);
    setTimeout(() => {
      fetchPosts(page, selectedValue.value).then((data: Posts) => {
  ...
  useEffect(() => {
    fetchPosts(1, selectedValue.value).then((data: Posts) => {
        ...
        {posts.map((post, index) => {
          return (
            <Post
              ...
              fullName={post.camera.full_name}
              landingDate={post.rover.landing_date}
              launchDate={post.rover.launch_date}
              imgSrc={post.img_src}
            />
          );
        })}
